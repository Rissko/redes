...
def receive():
    ...
    if message == "Ja existe um usuario com o mesmo nome":
        ...
        thread.start()
        thread = threading.Thread(target=serverLigacao.iniciarServidorLigacao, args=(str(split[1]), popup, endcallbtn, busy, console))
        thread.start()
...
def listen():
    ...
    while True:
        ...
        if "endereco" in message:
            ...
            dados = message.split('/')
            clientLigacao.iniciaConexaoUDP(dados[1], dados[3], endcallbtn, busy, console)
