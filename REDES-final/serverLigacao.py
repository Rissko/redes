...
def iniciarServidorLigacao(meuIp, callback, endcallbtn, busy, console):
    global servidorUdp
    global origem_call
        ...
        if "convite" in str(msg):
            if busy.flag['online']:
                ...
                if "s" in resp:
                    ...
                    thread2 = threading.Thread(target=enviarAudio, args=(busy,endcallbtn,console))
                    thread2.start()
                elif "n" in resp:
...
def enviarAudio(busy,endcallbtn,console):
    global py_audio
    global input_stream
    py_audio = pyaudio.PyAudio()
    buffer = 1024
